...
if (missionNamespace getVariable ["MSF_OFE_Init", false]) then {
	private _type = _logic getVariable ["Type", 5];
	...
	private _objs = nearestObjects [player, ["MSF_Placeholder"], _size];
	_objs append (nearestObjects [player, ["House_F"], _size]);
	[_logic, _objs, _type, [_vic, _vicChance, _supply]] call MSF_fnc_OFE_PopulatePosition;
};