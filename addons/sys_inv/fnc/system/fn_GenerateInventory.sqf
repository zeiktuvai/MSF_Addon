...
_inv merge ([] call MSF_Inv_fnc_GetPlayerWeaponInventory);
...
if (_fill) then {
	...
	while { loadAbs _object < _max } do {
		private _cat = [_weights] call MSF_Inv_fnc_SelectCategory;
		[_object, _inv get _cat] call MSF_Inv_fnc_AddItem;
		_remain = _remain - 1;
	};
} else {
