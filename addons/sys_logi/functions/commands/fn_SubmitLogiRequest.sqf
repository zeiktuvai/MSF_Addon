...
if (_airDrop == 1) then {
	_code = {
		...
		[missionNamespace, "MSF_Logi_DropRequested", [side _unit, _pos, _type]] remoteExec ["BIS_fnc_callScriptedEventHandler", 2];
...
else
{
	...
	[_code, _item, uiNamespace getVariable ["MSFLogiDeployCoord", []], _unit] spawn {
		params ["_code", "_item", "_pos", "_unit"];
		
		["MSF", "LastLogiRequstTime", serverTime] call MSF_fnc_SetConfigValue;
		sleep (random 30);
		[_code,[_item, _pos, _unit], objNull, true] call MSF_Logi_fnc_RequestLogistics;
	}
};
